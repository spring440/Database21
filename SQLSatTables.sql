IF EXISTS(SELECT * FROM sys.databases WHERE name = 'SQLSat')
...
CREATE TABLE Countries(
	...
	Two_Letter		VARCHAR(2) NOT NULL,
	Country_name	VARCHAR(50) NOT NULL,
	PRIMARY KEY (Country_id),
	CONSTRAINT UC_Countries UNIQUE(Country_name, Two_Letter)
);
GO

IF EXISTS (SELECT * FROM sys.tables WHERE name = 'Country_State')
--3
DROP TABLE Country_State;
CREATE TABLE Country_State(
	CS_id			INT IDENTITY NOT NULL,
	State_id		INT NOT NULL,
	Country_id		INT NOT NULL,
	PRIMARY KEY (CS_id),
	FOREIGN KEY (State_id) REFERENCES States_T(State_id),
...
CREATE TABLE Roles(
	...
	PRIMARY KEY (Role_id),
	CONSTRAINT UC_Roles UNIQUE (Role_name)
	);
...
CREATE TABLE Attendee(
	...
	FOREIGN KEY (Country_id) REFERENCES Countries(Country_id),
	FOREIGN KEY (State_id) REFERENCES States_T(State_id),
	CONSTRAINT UC_Attendee UNIQUE(F_name, L_name, A_address)
	);
GO


IF EXISTS (SELECT * FROM sys.tables WHERE name = 'Attendee_Role')
--6
DROP TABLE Attendee_Role;
...
IF EXISTS (SELECT * FROM sys.tables WHERE name = 'E_Location')
--7
DROP TABLE E_Location;
CREATE TABLE E_Location(
	Location_id			INT IDENTITY(1,1) NOT NULL,
	L_Name				VARCHAR(50) NULL,--Allows nulls in case venue has no name
	...
	FOREIGN KEY (Country_id) REFERENCES Countries(Country_id),
	FOREIGN KEY (State_id) REFERENCES States_T(State_id),
	CONSTRAINT UC_E_Locations UNIQUE(L_address, City)
	);
GO

IF EXISTS (SELECT * FROM sys.tables WHERE name = 'Difficulty')
--8		 
DROP TABLE Difficulty;
CREATE TABLE Difficulty(
	Diff_id				INT IDENTITY NOT NULL,
	Diff_title			VARCHAR(20)  NOT NULL,
	Diff_description	VARCHAR(128) NULL,
	PRIMARY KEY (Diff_id),
	CONSTRAINT UC_Difficulty UNIQUE (Diff_title)
	);
GO

IF EXISTS (SELECT * FROM sys.tables WHERE name = 'SS_Events')
--9
DROP TABLE SS_Events;
CREATE TABLE SS_Events(
	...
	FOREIGN KEY (Location_id) REFERENCES E_Location (Location_id),
	CONSTRAINT UC_SSL_Events UNIQUE (Event_num,Location_id, Event_date)
);
GO

IF EXISTS (SELECT * FROM sys.tables WHERE name = 'Event_tracks')
--10
DROP TABLE Event_tracks;
CREATE TABLE Event_tracks(
	Track_id				INT IDENTITY(1,1) NOT NULL,
	Track_name				VARCHAR(70) NOT NULL,
	Track_description		VARCHAR(256) DEFAULT'DESCRIPTION PENDING',
	PRIMARY KEY (Track_id),
	CONSTRAINT UC_Event_tracks UNIQUE (Track_name)
	);
GO


IF EXISTS (SELECT * FROM sys.tables WHERE name = 'Event_classes')
--11
DROP TABLE Event_classes;
CREATE TABLE Event_Classes(
	Class_id				INT IDENTITY(1,1) NOT NULL,
	Class_title				VARCHAR(50) NOT NULL,
	Class_description		VARCHAR(128) DEFAULT 'DESCRIPTION PENDING',
	Diff_id					INT NOT NULL,
	Track_id				INT NULL, --SOME CLASSES WILL NOT BE PART OF A TRACK
	Duration				INT DEFAULT '60',
	PRIMARY KEY (Class_id),
	FOREIGN KEY (Diff_id) REFERENCES Difficulty(Diff_id),
	FOREIGN KEY (Track_id) REFERENCES Event_tracks(Track_id),
	);
GO
...
CREATE TABLE Rooms(
	...
	Room_name		VARCHAR(50) NULL, --ALLOW NULL IN CASE ROOM HAS NO NAME
	Room_num		INT NOT NULL,
	Location_id		INT NOT NULL,
	Max_occ			INT NOT NULL,
	PRIMARY KEY (Room_id),
	FOREIGN KEY (Location_id) REFERENCES E_Location (Location_id),
	CONSTRAINT UC_Rooms UNIQUE (Room_num,Location_id)
	);
GO
	
--13
IF EXISTS (SELECT * FROM sys.tables WHERE name = 'Event_Schedule')
DROP TABLE Event_Schedule;
CREATE TABLE Event_Schedule(
	Schedule_id			INT IDENTITY(1,1) NOT NULL,
	Event_id			INT NOT NULL,
	Class_id			INT NOT NULL,
	Class_time			TIME DEFAULT '00:00',
	Room_id				INT DEFAULT '0',
	AR_id				INT NOT NULL,
	PRIMARY KEY (Schedule_id),
	FOREIGN KEY (Event_id) REFERENCES SS_Events(Event_id),
	FOREIGN KEY (Class_id) REFERENCES Event_Classes (Class_id),
	FOREIGN KEY (Room_id) REFERENCES Rooms (Room_id),
	FOREIGN KEY (AR_id) REFERENCES Attendee_Role (AR_id),
	CONSTRAINT UC_Event_Schedule UNIQUE (Room_id,Class_id,Event_id)
	);
GO	

IF EXISTS (SELECT * FROM sys.tables WHERE name = 'Sponsor_Type')
--14
DROP TABLE Sponsor_Type;
CREATE TABLE Sponsor_Type(
	SP_id			INT IDENTITY(1,1) NOT NULL,
	Type_Title		VARCHAR(25) NOT NULL,
	Type_Descr		VARCHAR(128) NULL,
	PRIMARY KEY (SP_id),
	CONSTRAINT UC_Sponsor_Type UNIQUE (Type_Title)
);
GO
...
CREATE TABLE Sponsors(
	...
	FOREIGN KEY (SP_id) REFERENCES Sponsor_Type(SP_id),
	CONSTRAINT UC_Sponsors UNIQUE (Sponsor_Name)
	);
GO


IF EXISTS (SELECT * FROM sys.tables WHERE name = 'Event_Tables')
--16
DROP TABLE Event_Tables;
CREATE TABLE Event_Tables(
	...
	CONSTRAINT UC_Event_Tables UNIQUE (Table_Num, Event_id, Sponsor_id)
	);
GO

IF EXISTS (SELECT * FROM sys.tables WHERE name = 'Raffles')
--17
DROP TABLE Raffles;
CREATE TABLE Raffles(
	...
	Sponsor_id			INT NOT NULL,
	Raffle_Item			VARCHAR (128) NOT NULL,
...
IF EXISTS (SELECT * FROM sys.tables WHERE name = 'Raffle_Guests')
--18
DROP TABLE Raffle_Guests;
CREATE TABLE Raffle_Guests(
	RG_id				INT IDENTITY(1,1) NOT NULL,
	Raffle_id			INT NOT NULL,
	Attendee_id			INT NOT NULL, 
	Lecture_Grade		VARCHAR(2) NOT NULL,
	PRIMARY KEY (RG_id),
	FOREIGN KEY (Raffle_id) REFERENCES Raffles (Raffle_id),
	FOREIGN KEY (Attendee_id) REFERENCES Attendee (Attendee_id),
	CONSTRAINT UC_Raffle_Guests UNIQUE (RG_id, Raffle_id, Attendee_id)
	);
GO
...
CREATE TABLE Presentation(
	...
	Presentation_title		VARCHAR(128) NOT NULL,
